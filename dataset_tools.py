...
def open_stl10(tarball: str, *, max_images: Optional[int]):
    ...
    with tarfile.open(tarball, "r:gz") as tar:
        ...
        with tar.extractfile(member) as file:
            data = pickle.load(file, encoding="latin1")
        images.append(data["data"].reshape(-1, 3, 64, 64))
        labels.append(data["labels"])

    images = np.concatenate(images)
    labels = np.concatenate(labels)
    images = images.transpose([0, 2, 3, 1])  # NCHW -> NHWC
    assert images.shape == (50000, 64, 64, 3) and images.dtype == np.uint8
    assert labels.shape == (50000,) and labels.dtype in [np.int32, np.int64]
    assert np.min(images) == 0 and np.max(images) == 255
    assert np.min(labels) == 0 and np.max(labels) == 9
...
def open_dataset(source, *, max_images: Optional[int]):
    if os.path.isdir(source):
        ...
        if os.path.basename(source) == "stl10_binary.tar.gz":
...
def convert_dataset(
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        idx_str = f"{idx:08d}"
        archive_fname = f"img{idx_str}.jpg"
