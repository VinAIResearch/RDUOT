...
def sample_and_test(args, epoch_id):
    ...
    if args.perturb_percent > 0:
        ...
        result_dir += f"_{int(args.perturb_percent)}p_{args.perturb_dataset}"
        save_dir += f"_{int(args.perturb_percent)}p_{args.perturb_dataset}"

    samples_folder = samples_dir + f"/{args.version}"
    result_folder = result_dir + f"/{args.version}/fid_1.12"
    save_dir += f"/{args.version}"
    result_dir += f"/{args.version}/netG_{epoch_id}.pth"
    ...
    json_filename = f"{result_folder}/stat.json"
    print(json_filename)

    netG = NCSNpp(args).to(device)
    ckpt = torch.load(result_dir, map_location=device)
        ...
        for i in range(iters_needed):
            with torch.no_grad():
                ...
                for j, x in enumerate(fake_sample):
                    ...
                    torchvision.utils.save_image(x, f"{samples_folder}/{index}.jpg")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--master_port", type=str, default="6020", help="master port number")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1800,
        help="cosine scheduler, learning rate 1e-5 until {schedule} epoch",
    )
